...
namespace BBTimes.ModPatches
{
	...
	internal class MathMachinePatches
	{
		[HarmonyPrefix]
		[HarmonyPatch("Start")]
		private static void AdaptiveText(MathMachine __instance) =>
			__instance.transform.Find("Answer").GetComponent<TextMeshPro>().autoSizeTextContainer = true; // 10+ answers don't look ugly
		

		[HarmonyPostfix]
		[HarmonyPatch("Completed")]
		private static void WOOOW(ref AudioManager ___audMan, bool ___givePoints)
		{
		...
		private static IEnumerable<CodeInstruction> ExtraNumballs(IEnumerable<CodeInstruction> instructions) =>
			...
			.SetInstruction(Transpilers.EmitDelegate(() => BBTimesManager.CurrentFloorData.MathNumberAmount.RandomVal + 1))
